...
for step in Functions.frange(0, 2, 0.05):
    ...
    for imagePath in imagePaths:
        ...
        # Load the image to be classified, and blur it with a gaussian blur
        if step != 0:
            imageT = Image.open(imagePath)
            imageT = imageT.filter(ImageFilter.GaussianBlur(radius=step))
...
#plt.title("Accuracy")
#plt.xlabel("Image alteration - Blur")
plt.ylabel("Accuracy")
plt.savefig("accuracy_plot_blur.jpg")
