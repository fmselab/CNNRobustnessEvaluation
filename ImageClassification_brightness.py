...
for step in Functions.frange(-1, 1, 0.05):
    ...
    for imagePath in imagePaths:
        ...
        if float(step) > 0.000001 or float(step) < -0.000001:
            image = Image.open(imagePath)
            enhancer = ImageEnhance.Brightness(image)
            image = enhancer.enhance(1+(step*0.5))
            image = np.array(image)
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            steps[steps.__len__() - 1] = step * 50
        else:
            image = cv2.imread(imagePath)
            steps[steps.__len__() - 1] = 0
...
#plt.title("Accuracy")
#plt.xlabel("Image alteration - Brightness")
plt.ylabel("Accuracy")
plt.savefig("accuracy_plot_brightness.jpg")
