	...
	if step_img % 2 == 0:
		...
		for step in Functions.frange(0, 1, 0.5):
			if step < -0.0001 or step > 0.0001:
				...
				image = Functions.crop_around_center(image,
													 *Functions.largest_rotated_rect(50, 50, math.radians(180 * step)))
				try:
					...
					print("[INFO - Rotation] Picture empty (" + str(num_img) + ")")
		...
		for step in Functions.frange(-1, 1, 0.5):
			if step < -0.0001 or step > 0.0001:
				...
				# Translate the picture, by cropping some portions on the left and on the right
				image = image [5+int(step*5):44+int(step*5), :]
				...
				try:
					...
					print("[INFO - Vertical Translation] Picture empty (" + str(num_img) + ")")
		...
		for step in Functions.frange(-1, 1, 0.5):
			if step < -0.0001 or step > 0.0001:
				...
				try:
					...
					print("[INFO - Horizontal Translation] Picture empty (" + str(num_img) + ")")
		...
		for step in Functions.frange(0, 1, 0.5):
			if float(step) > 0.0001 or float(step) < -0.0001:
				...
				# Zoom the image and get its dimensions
				image = scipy.misc.imresize(image, float(1 + step))
				rows, cols, ch = image.shape

				# Crop the image
				image = image[int(rows / 2) - 25:int(rows / 2) + 25, int(cols / 2) - 25:int(cols / 2) + 25]
				...
				try:
					...
					print("[INFO - Zoom] Picture empty (" + str(num_img) + ")")
	step_img = step_img + 1
...
plt.savefig("loss_plot_augmented.jpg")
