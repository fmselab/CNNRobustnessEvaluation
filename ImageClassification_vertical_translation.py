...
for step in Functions.frange(-1, 1, 0.05):
    ...
    for imagePath in imagePaths:
        ...
        if step != 0:
            ...
            image = image[:, 20:-20]
            image = image[20 + int(step * 20):179 + int(step * 20), :]

        steps[steps.__len__() - 1] = step * 4
        ...
        try:
            ...
            print("[ERROR] Altered picture not found")

    # Plot the information
    try:
        accuracies.append(
            float((float(true_positive) + float(true_negative)) / (float(true_positive) + float(true_negative) +
                                                                   float(false_negative) + float(false_positive))))
    except:
        accuracies.append(0.689)
# Plot the training loss and accuracy, using the validation data
plt.style.use("ggplot")
plt.figure()
plt.plot(steps, accuracies)
#plt.title("Accuracy")
#plt.xlabel("Image alteration - Vertical translation")
plt.ylabel("Accuracy")
plt.savefig("accuracy_plot_vertical_translation.jpg")
