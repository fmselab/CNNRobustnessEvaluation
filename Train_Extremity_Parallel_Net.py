...
for imagePath in imagePaths:
	...
	# 1. Compression -> Train the net only on the extremity, so only with the most compressed image
	...
	# 2. Gaussian Noise -> Train the net only on the extremity, so only with the most noisy image
	image = cv2.imread(imagePath)
	gaussian_noise = image.copy()
	cv2.randn(gaussian_noise, 0, 200)
	...
	# 3. Brightness variation -> Train the net only on the extremity, so only with the most underexposed and overexposed
	# picture
	...
	# 4. Blur addition -> Train the net only on the extremity, so only with the most blurred image
	imageT = Image.open(imagePath)
	imageT = imageT.filter(ImageFilter.GaussianBlur(radius=2))
...
plt.savefig("loss_plot_parallel_extremity.jpg")
