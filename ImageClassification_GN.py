...
for step in Functions.frange(0, 200, 5):
    ...
    for imagePath in imagePaths:
        ...
        # Load the image to be classified, and add a gaussian noise
        if step <0.0001 or step > 0.0001:
            image = cv2.imread(imagePath)
            gaussian_noise = image.copy()
            cv2.randn(gaussian_noise, 0, step)
...
#plt.title("Accuracy")
#plt.xlabel("Image alteration - Gaussian Noise")
plt.ylabel("Accuracy")
plt.savefig("accuracy_plot_GN.jpg")
